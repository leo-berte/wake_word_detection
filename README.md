...
$$
h_t^{hat} = \tanh(W_h x_t + r_t \odot (U_h h_{t-1}) + b_h)
$$

$$
h_t = (1 - z_t) \odot h_{t-1} + z_t \odot h_t^{hat}
$$
...
* $h_t$: Hidden state at time $t$ with dimensions $(sequence\_length \times hidden\_size)$
* $x_t$: Input vector at time $t$ with dimensions $(sequence\_length \times input\_size)$
* $h_t^{hat}$: Candidate hidden state with dimensions $(sequence\_length \times hidden\_size)$
...
$$
c_t^{hat} = \tanh(W_c x_t + U_c h_{t-1} + b_c)
$$

$$
c_t = f_t \odot c_{t-1} + i_t \odot c_t^{hat}
$$
...
* $c_t^{hat}$: Candidate cell state with dimensions $(sequence\_length \times hidden\_size)$
* $x_t$: Input vector at time $t$ with dimensions $(sequence\_length \times input\_size)$
